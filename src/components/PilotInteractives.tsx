...
export function ScalingLab() {
  ...
  return <div className="not-prose my-6 rounded-2xl border bg-card p-5 sm:p-7"><label className="text-sm font-bold">Annual income: ?{income.toLocaleString()}</label><input aria-label="Annual income" type="range" min="12000" max="98000" step="1000" value={income} onChange={e=>setIncome(Number(e.target.value))} className="mt-3 w-full accent-primary"/><div className="mt-5 grid gap-3 sm:grid-cols-2"><div className="rounded-xl bg-indigo-500/10 p-4"><p className="text-xs font-bold uppercase tracking-wider text-indigo-600 dark:text-indigo-300">Min?max [0,1]</p><p className="mt-2 text-3xl font-black">{minmax.toFixed(3)}</p><p className="mt-1 text-xs text-muted-foreground">Position inside the observed range</p></div><div className="rounded-xl bg-emerald-500/10 p-4"><p className="text-xs font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-300">Z-score</p><p className="mt-2 text-3xl font-black">{z.toFixed(2)}?</p><p className="mt-1 text-xs text-muted-foreground">Distance from the mean in standard deviations</p></div></div></div>;
}
